...
<%
	...
	if(request.getAttribute("buildingIdList")!=null){
		buildingIdList=(ArrayList)request.getAttribute("buildingIdList");
	}
...
<div class="btn-toolbar list-toolbar">
   <button class="btn btn-primary" onclick="addEvent()"><i class="fa fa-plus"></i>添加新的电力信息</button>
   <button class="btn btn-primary" onclick="k2()"><i class="fa fa-plus"></i>搜索</button>
   <button class="btn btn-primary" onclick="k3()"><i class="fa fa-plus"></i>图表页面</button>
...
<div  class="scroller" style="overflow-y:auto;height: 440px;" id="kkk">   
	<table class="table" style=" -webkit-user-select:none;
	  ...
	  <tbody style="overflow-y:scroll;">
	    ...
	    <tr>
	      <td><%=temp.getPowerId() %></td>
	      <td><%=temp.getTotalconsumption() %></td>
	      <td><%=temp.getSubsidyconsumption() %></td>
	      <td><%=temp.getPayconsumption() %></td>
	      <td><%=temp.getFreeconsumption() %></td>
	      <td><%=temp.getPrice() %></td>
	      <td><%=temp.getPowerBuildingId() %></td>
	      <td>
...
<div class="hide-center">
    <div id="formhead" style="width:640px !important;">
        <div id="formhead-title"  style="position:absolute; left:170px; margin-left: 80px; width: 100px; padding-left: 15px;">新增电力信息</div>
        <button type="button" id="close" style="float:right">X</button>
    </div>
    <div id="formbody" style="height:250px !important; width: 640px !important;">
    <form action="AddNewPower" id="form" style="width: 640px !important;">
        <div class="loginUserName"  style="padding-left: 15px;">
         	<input style="width: 200px;" onclick="test1()" id="powerBuildingId" name="powerBuildingId" class="loginInput" placeholder="公寓id" type="text" value="">
            <input style="width: 200px; padding-left: 20px;" onclick="test2()" id="totalconsumption" name="totalconsumption" class="loginInput" placeholder="总电量" type="text" value="">
            <input style="width: 200px; padding-left: 20px;" onclick="test3()" id="subsidyconsumption" name="subsidyconsumption" class="loginInput" placeholder="补助电量" type="text" value="">
        </div>
        <div class="loginUserName" style="padding-left: 15px;">
       		<input style="width: 200px;" onclick="test4()" id="freeconsumption" name="freeconsumption" class="loginInput" placeholder="免费电量" type="text" value="">
            <input style="width: 200px; padding-left: 20px;" onclick="test5()" id="payconsumption" name="payconsumption" class="loginInput" placeholder="花费电量" type="text" value="">
            <input style="width: 200px; padding-left: 20px;" onclick="test6()" id="price" name="price" class="loginInput" placeholder="价格" type="text" value="">
        </div>
         <input id="powerId" name="powerId" class="loginInput" type="hidden" value="">
        <div id="formfoot">
...
<div class="hide-center">
	...
	color:#bfbfbf;
	font-weight:900;
	display:inline-block;">查找电力信息 </div>
    ...
    <form action="SearchPower" id="form">
        <div class="loginUserName">
            <input id="buildingId" name="buildingId"  onclick="test7()"  class="loginInput" placeholder="宿舍号" type="text" value="">
        </div>
        <div id="formfoot">
...
<!-- 登录弹出框结束 -->
<!-- 登录弹出框 -->
<div id="findTB" style="display: none; position: absolute; left: 240px; height: 170px;" >
<div class="hide-center">
    <div id="formhead" style="width:300px;height:42px;
	margin:0;
	padding-top:12px;
	...
	color:#bfbfbf;
	font-weight:900;
	display:inline-block;">公寓楼号 </div>
        <button type="button" id="closeoo" style="width:35px;
	height:20px;
	border:0;
	margin-left:45px;
	border-radius:20%;
    ...
    <div id="formbody" style="width:300px ;height:170px ">
    <form action="StuGetPower" id="form">
        <div class="loginUserName">
            <!-- <input id="buildingId" name="buildingId"  onclick="test7()"  class="loginInput" placeholder="公寓楼号" type="text" value=""> -->
            <select id="buildingId" name="buildingId" class="loginInput">
 				<%for(int i=0;i<buildingIdList.size();i++){ %>
 				<option value ="<%=buildingIdList.get(i) %>"><%=buildingIdList.get(i) %>号楼</option>
 				<%} %>
			</select>
        </div>
        <div id="formfoot">
...
function test1(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('powerBuildingId', 'softkey1');
...
function test2(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('totalconsumption', 'softkey2');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=2;
	time=time+1;
}
function test3(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('subsidyconsumption', 'softkey3');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=3;
	time=time+1;
}
function test4(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('freeconsumption', 'softkey4');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=4;
	time=time+1;
}
function test5(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('payconsumption', 'softkey5');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=5;
	time++;
}
function test6(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('price', 'softkey6');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=6;
	time++;
}
function test7(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('buildingId', 'softkey7');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=7;
	time++;
}
...
function addEvent(){
	document.getElementById('formhead-title').innerHTML="新增电力信息";
    document.getElementById('form').action="AddNewPower";
    document.getElementById('BSignIn').innerHTML="添加";
    $('#powerId').removeAttr("readonly");
    $('#powerId').removeAttr("disabled"); 
    $('#totalconsumption').removeAttr("disabled"); 
    $('#subsidyconsumption').removeAttr("disabled"); 
    $('#payconsumption').removeAttr("disabled"); 
    $('#freeconsumption').removeAttr("disabled"); 
    $('#price').removeAttr("disabled"); 
    $('#powerBuildingId').removeAttr("disabled"); 
    
    document.getElementById('powerId').value="";
    document.getElementById('totalconsumption').value="";
    document.getElementById('subsidyconsumption').value="";
    document.getElementById('payconsumption').value="";
    document.getElementById('freeconsumption').value="";
    document.getElementById('price').value="";
    document.getElementById('powerBuildingId').value="";
	k1();
}
function k2() {
...
function k3() {
	document.getElementById('t1').style.display="none";
	document.getElementById('findTB').style.display="block";
	document.getElementById('softkey7').style.display="block";
    $(".hide-center").fadeIn("slow");
    $(".overCurtain").fadeIn("slow");
    document.getElementById('login').style.display="none";
...
$("table").on("click","tr",function(e){
	   ...
	   if($(e.target).attr("class")=="fa fa-delete"||
		      ...
		      document.getElementById('form').action="DeletePower";
		      document.getElementById('BSignIn').innerHTML="删除";
		      document.getElementById('powerId').readonly="readonly";
		      document.getElementById('totalconsumption').disabled="true";
		      document.getElementById('subsidyconsumption').disabled="true";
		      document.getElementById('payconsumption').disabled="true";
		      document.getElementById('freeconsumption').disabled="true";
		      document.getElementById('price').disabled="true";
		      document.getElementById('powerBuildingId').disabled="true";
		  }else if($(e.target).attr("class")=="fa fa-pencil"){
			  document.getElementById('formhead-title').innerHTML="修改电力信息";
		      document.getElementById('form').action="ExchangePower";
		      document.getElementById('BSignIn').innerHTML="修改";
		      document.getElementById('powerBuildingId').readonly="readonly";
		      /* alert($(e.target).attr("class")); */
		  }
       if(document.getElementById('formhead-title').innerHTML=="新增电力信息"){
           ...
           document.getElementById('powerId').value=arr[0];
		   document.getElementById('totalconsumption').value=arr[1];
		   document.getElementById('subsidyconsumption').value=arr[2];
		   document.getElementById('payconsumption').value=arr[3];
		   document.getElementById('freeconsumption').value=arr[4];
		   document.getElementById('price').value=arr[5];
		   document.getElementById('powerBuildingId').value=arr[6];
          /*  alert(arr); */
           k1();
       }
    })
