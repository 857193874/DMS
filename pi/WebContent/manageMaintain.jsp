...
<%
	...
	if(request.getAttribute("MaintainAll")!=null){
		arr=(ArrayList)request.getAttribute("MaintainAll");
	}
	Maintain temp=new Maintain();
%>
...
<div class="btn-toolbar list-toolbar">
   <button class="btn btn-primary" onclick="addEvent()"><i class="fa fa-plus"></i>添加报修信息</button>
...
<div  class="scroller" style="overflow-y:auto;height: 440px;" id="kkk">     
	<table class="table" style=" -webkit-user-select:none;
	  ...
	  <tbody>
	    ...
	    temp=(Maintain)arr.get(i);%>
	    <tr>
	      <td><%=temp.getMaintainId() %></td>
	      <td><%=temp.getDomitoryId() %></td>
	      <td><%=temp.getMaintainTime() %></td>
...
<div class="hide-center">
    <div id="formhead"  style="width:300px !important;">
        <div id="formhead-title" style="margin-left: 80px; width: 100px; padding-left: 15px;">新增报修信息</div>
        <button type="button" id="close" style="float:right">X</button>
    </div>
    <div id="formbody" style="height:250px !important; width: 300px !important;">
    <form action="AddNewMaintain" id="form" style="width: 240px !important;">
        <div class="loginUserName" style="padding-left: 30px;">
            <input style="width: 230px;" onclick="test1()" id="domitoryId" name="domitoryId" class="loginInput" placeholder="宿舍号" type="text" value="">
        </div>
         <div class="loginUserName" style="padding-left: 30px;">
            <input style="width: 230px;" onclick="test2()" id="status" name="status" class="loginInput" placeholder="状态" type="text" value="">
        </div>
        <div id="formfoot">
            <button id="BSignIn" type="submit"  style="width: 230px;">添加</button>
        </div>
        </form>
    </div>
</div>
...
<div class="hide-center">
	...
	color:#bfbfbf;
	font-weight:900;
	display:inline-block;">查找报修信息 </div>
    ...
    <form action="SearchMaintain" id="form">
        <div class="loginUserName">
            <input id="domitoryId1" onclick="test3()" name="domitoryId" class="loginInput" placeholder="宿舍号" type="text" value="">
        </div>
        <div id="formfoot">
...
function test1(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('domitoryId', 'softkey1');
...
function test2(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('status', 'softkey2');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=2;
	time=time+1;
}
function test3(){
	$(".keyboard").fadeIn(0);
	VirtualKeyboard.toggle('domitoryId1', 'softkey3');
	$("#kb_langselector,#kb_mappingselector,#copyrights").css("display", "none");
	k=3;
	time=time+1;
}
...
function addEvent(){
	document.getElementById('formhead-title').innerHTML="新增报修信息";
    document.getElementById('form').action="AddNewMaintain";
    document.getElementById('BSignIn').innerHTML="添加";
    $('#domitoryId').removeAttr("readonly");
    
    document.getElementById('domitoryId').value="";
...
$("table").on("click","tr",function(e){
	/* alert($(e.target).attr("class")); */
	   if($(e.target).attr("class")=="fa fa-pencil"){
			  document.getElementById('formhead-title').innerHTML="修改报修信息";
		      document.getElementById('form').action="AddNewMaintain";
		      document.getElementById('BSignIn').innerHTML="修改";
		      document.getElementById('domitoryId').readonly="readonly";
		  }
       if(document.getElementById('formhead-title').innerHTML=="新增报修信息"){
    	   
       }else if(document.getElementById('formhead-title').innerHTML=="修改报修信息"){
    	   var arr = []
           $(this).children().map(function(el){
               arr.push($(this)[0].innerText)
           })
           document.getElementById('domitoryId').value=arr[1];
           document.getElementById('status').value=arr[3];
           /* alert(arr); */
           k1();
       }
    })
